package com.linkedin.collections.module03.video07.challenge.iterating.collections;
...
public class RoomService {
	...
	public void applyDiscount(final double discount) {
		...
		//double calcDiscountDecimal = 1-discount/100;  //.00
		double calcDiscountDecimal = 1-discount;  //.00
		inventory.stream()
				.forEach(room -> room.setRate(room.getRate() * calcDiscountDecimal));
	...
	public Collection<Room> getRoomsByCapacity(final int requiredCapacity) {
		...
		return inventory.stream().filter(r->r.getCapacity()>=requiredCapacity).collect(Collectors.toList());
	}
	...
	public Collection<Room> getRoomByRateAndType(final double rate, final String type){
		...
		return inventory.stream().filter(r -> r.getType().equals(type) && r.getRate()<= rate).collect(Collectors.toList());
	}
